...
/**
 * Generate Java records from Oracle Domain Definitions.
 ...
 * @param include Optional include filters (object names or patterns)
 * @param outputDir Output directory for generated sources (not auto-attached to source sets)
 */
@PluginTask
public record GenerateOracleDomainsTask(
    @PluginTaskParameter
    ...
    /**
     * Execute domain-based code generation.
    ...
    public void generate() {
        ...
        // NOTE: Real implementation should connect to Oracle and introspect DOMAIN DEFINITIONS.
        ...
        try {
            ...
            String simpleName = "OracleDomainSample";
...
/**
 * Generated from Oracle Domain Definitions (scaffold).
 * Field set is derived from Oracle metadata (domains). This placeholder emits an empty record.
 */
@JsonSchema
public record %s() {
}
""".formatted(pkg, simpleName);
            ...
            if (Boolean.TRUE.equals(failOnMissingDb)) {
                throw new RuntimeException("Oracle domain generation failed", e);
            }
            System.err.println("Oracle domain generation skipped due to error (failOnMissingDb=false): " + e.toString());
        }
    }
}
