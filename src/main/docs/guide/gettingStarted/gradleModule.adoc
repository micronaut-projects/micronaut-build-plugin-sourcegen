...
You can see the javadoc for the generated sources in this example in link:{api}/io/micronaut/sourcegen/example/plugin/gradle/package-summary.html[io.micronaut.sourcegen.example.plugin.maven package javadoc].

See documentation for link:{api}/io/micronaut/sourcegen/annotations/GenerateGradlePlugin.html[GenerateGradlePlugin] to view all the configurable properties.

=== Plugin Customization

Plugin and extension can be extended to add custom Gradle-specific behavior.

snippet::io.micronaut.sourcegen.example.plugin.gradle.TestExtensionImpl[project-base="test-suite-gradle", tags="begin,generateRecordWithName,createGenerateSimpleRecordTask,end", source="main"]
