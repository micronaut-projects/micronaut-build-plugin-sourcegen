...
Only `AbstractGenerateSimpleRecordMojo` class will be generated. The mojo will have all the specified task parameters and will call the defined task as its action. You can see the javadoc for the generated sources in this example in link:{api}/io/micronaut/sourcegen/example/plugin/maven/package-summary.html[io.micronaut.sourcegen.example.plugin.maven package javadoc].
